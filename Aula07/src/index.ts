...
function addCrewMemeber(member: string, spaceship: { name: string, crewLimit: number, crew: string[] }) {
...
function firstMenuOption() {
    ...
    const confirmation = confirm(`Confirma o registro da name ${name}\n Piloto: ${pilot}\n Limite da tripulação: ${crewLimit}`)
...
function secondMenuOption() {
    ...
    if (spaceship) {
        const confirmation = confirm(`Confirma a inclusão da ${member} na tripulação da ${spaceship.name}?`)

        if (confirmation) {
            addCrewMemeber(member, spaceship)
        }
    }
}
